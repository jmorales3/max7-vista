...
- `users.force_password_change`, `mfa_enabled`, `mfa_secret`, `mfa_backup_codes` — existed in Postgres schema/users.ts (and even in the SQLite *Drizzle* schema file for the mfa_* columns) but were never added to the real SQLite `CREATE TABLE users` or `sqliteMigrations` — caused every SQLite login to throw "no such column: mfa_enabled" (surfaced as generic 500 "Internal server error"). Gap existed because the Drizzle schema file and the raw-SQL migration list can drift independently — a column can be "half-added" (declared in schema-sqlite/users.ts but not in the actual CREATE TABLE/migrations), which is worse than being fully missing since it silently passes typecheck. Fixed in v1.1.6.
