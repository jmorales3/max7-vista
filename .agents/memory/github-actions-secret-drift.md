...
5. STANDING DIRECTIVE for this project: run `.local/scripts/verify-secret-sync.mjs` (dispatches `.github/workflows/verify-license-secret.yml`, HMACs a random nonce with each side's secret, compares digests — never exposes either raw value) before every tagged release that touches licensing. Treat a mismatch as a release blocker, same tier as the existing `check-github-sync.mjs` history-divergence check.
