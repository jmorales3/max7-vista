...
- Gallery: Browse all images with 1/2/4/8-per-row grid views. Filter by patient or show unassigned images. Also shows Image Library assets (decorative images) when "All Patients" is selected, so both clinical and library images live in one browsable view. Tag Filtering: click one or more tag pills at the top to filter the gallery to images (patient and library) carrying any of the selected tags (union match); click "Show All" to clear filters. Selection: click "Select" to enter selection mode, then click the checkmark on any tile to select images/videos across patients and library. With items selected, "Create Presentation" builds a new presentation (optionally scoped to a patient) or appends the selection to an existing one. Export: with items selected, the Export menu offers "Download as ZIP" (single archive) or "Download Individually" (separate file downloads). Presentation Export: from the Presentations hub, each presentation card has an export icon offering "Export as PDF" or "Export as PowerPoint" — both render every slide (single image, video thumbnail, before/after compare, superimpose overlay) to a file you can share outside the app.
...
- License & Activation (desktop/Electron version only): Max7 Vista desktop starts with a 30-day free trial. The current license status is shown in Settings → License section: states are Trial (days remaining shown), Active, Trial Expired, Expired, or Tampered. When the trial or a paid license expires, or if tampering is detected, a blocking activation screen appears at startup. To activate: copy the Machine ID shown in Settings → License, send it to your Max7 Vista vendor to receive an activation code, paste the code into the activation field, and click Activate. The activation code is tied to this specific machine and cannot be transferred. Active licenses show the plan name and expiry date (or "Lifetime" if perpetual). A yellow banner appears in the app when fewer than 7 days remain in a trial; a red banner appears when the trial has expired.
